<?php
	...
	$query1="SELECT COLUMN_NAME, DATA_TYPE
			FROM mi_produzione.INFORMATION_SCHEMA.COLUMNS
	...
	{
		echo "<table id='myTable$table' class='editableTableMiDbTecnico'>";
			echo "<tr>";
				foreach ($columns as $column)
				{
					$foreignKeyColumn="";
					...
					echo '<th style="border-right:none" class="editableTableMiDbTecnicoColumnNameTh">';
						echo $column;
					echo '</th>';
					echo '<th style="border-left:none" class="editableTableMiDbTecnicoFilterTh">';
					$visibility="";
					if (in_array($column, $noFilterColumns))
						$visibility="visibility:hidden";
						echo '<i style="cursor:pointer;float:right;'.$visibility.'" class="fal fa-filter btnFilter" id="editableTableMiDbTecnicoBtnFilter'.$column.'" onclick="openContextMenu(event,'.htmlspecialchars(json_encode($column)).')"></i>';
			...
			{
				echo "<tr id='editableTableMiDbTecnicoRow$i' class='editableTableMiDbTecnicoRow'>";
					foreach ($columns as $column) 
					{

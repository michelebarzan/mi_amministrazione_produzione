...
var noFilterColumns=[];
var rowCells=[];;
var editableTableMiDbTecnicoHeaders=[];
var selectetTable;
...
function getEditableTable()
{
	...
	checkboxId=[];
	rowCells=[];
	editableTableMiDbTecnicoHeaders=[];
	...
	$.post("editableTableMiDbTecnico/getTableColumns.php",
	{
		table
	},
	function(response, status)
	{
		if(status=="success")
		{
			...
			else
			{
				...
				$.post("editableTableMiDbTecnico/getEditableTable.php",
				{
					JSONnoFilterColumns,
					JSONcolumns,
					JSONreadOnlyColumns,
				...
				function(response, status)
				{
					if(status=="success")
					{
						...
						document.getElementById("myTable"+table).addEventListener("load", editableTableMiDbTecnicoLoad());
						editableTableMiDbTecnicoHeaders=[];
						var all=document.getElementsByClassName("editableTableMiDbTecnicoColumnNameTh");
						for (var i = 0; i < all.length; i++) 
						{
							editableTableMiDbTecnicoHeaders.push(all[i].innerHTML);
						}
...
function insertNewRow(table)
{
	...
	row.setAttribute("id","editableTableMiDbTecnicoRow"+newIndex);
	row.setAttribute("class","editableTableMiDbTecnicoRow");
...
function insertRecord(row,table,newIndex)
{
	...
	$.post("editableTableMiDbTecnico/insertRecordEditableTable.php",
	{
		table,
...
function setRowEditable(primaryKey,index,table,primaryKeyValue)
{
	...
	{
		var colName=editableTableMiDbTecnicoHeaders[j];
...
function deleteRow(primaryKey,table,primaryKeyValue,index)
{
	...
	$.post("editableTableMiDbTecnico/deleteRowEditableTable.php",
	{
		table,
		primaryKey,
		primaryKeyValue
...
function confirmUpdate(index,table,oldRowButtons,primaryKey,primaryKeyValue)
{
	...
	for (var j = 0; j<(table2.rows[index].cells.length)-1; j++)
	{
		...
		if(!readOnlyColumns.includes(colName))
		{
			updateValuesAndIndices[editableTableMiDbTecnicoHeaders[j]]=table2.rows[index].cells[j].childNodes[0].value;
		}
	}
	...
	newGridSpinner("","buttonColumn"+index,"width:24px;margin:0","","");
	$.post("editableTableMiDbTecnico/confirmUpdateEditableTable.php",
	{
		table,
		primaryKey,
		primaryKeyValue,
	...
	function(response, status)
	{
		if(status=="success")
		{
			...
			{
				for (var j = 0; j<editableTableMiDbTecnicoHeaders.length; j++)
				{
					var colName=editableTableMiDbTecnicoHeaders[j];
					if(!readOnlyColumns.includes(colName))
					{
...
{
	var all = document.getElementsByClassName("editableTableMiDbTecnicoRow");
	for (var i = 0; i < all.length; i++) 
	{
		all[i].style.display='';
	}
	...
	for (var i = 1, row; row = table2.rows[i]; i++)
	{
		...
		columns.forEach(function(colonna)
		{
			if(!noFilterColumns.includes(colonna))
			{
				...
				{
					document.getElementById('editableTableMiDbTecnicoRow'+i).style.display="none";
	...
	columns.forEach(function(colonna)
	{
		if(!noFilterColumns.includes(colonna))
		{
			if(inValues[colonna].length!=inValuesAll[colonna].length)
				document.getElementById("editableTableMiDbTecnicoBtnFilter"+colonna).style.color="red";
			else
				document.getElementById("editableTableMiDbTecnicoBtnFilter"+colonna).style.color="";
		}
	});
}
